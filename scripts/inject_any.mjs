...
// 通用注入器：把一个 Markdown 审稿稿转成某模块某章的 sections 写回 seed-content.json
// 支持：任意模块(frontend/backend/devops)；章不存在时自动创建。
// 用法: node inject_any.mjs <moduleId> <chapterId> <mdFile>
// md 约定:
//   <!-- title: 章节干净标题 -->
//   <!-- goal: 章节目标 -->
//   # <sectionId> | <section标题>
...
const [, , modId, chId, mdPath] = process.argv;
if (!modId || !chId || !mdPath) {
  console.error('usage: node inject_any.mjs <moduleId> <chapterId> <mdFile>');
  process.exit(1);
}
...
if (!mod) { console.error('module not found:', modId); process.exit(1); }
let ch = mod.chapters.find((c) => c.id === chId);
const isNew = !ch;
if (isNew) ch = { id: chId, title: chId, goal: '', sections: [] };
...
if (isNew) mod.chapters.push(ch);
fs.writeFileSync(SEED, JSON.stringify(s, null, 2));
console.log(`[${isNew ? 'NEW' : 'UPDATE'}] ${modId}/${chId}: title="${ch.title}" sections=${ch.sections.length}`);
