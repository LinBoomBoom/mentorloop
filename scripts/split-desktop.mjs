...
function buildSplit(origById) {
  ...
  const newChapters = [
    ...
    { id: 'dt-c2', title: o('dt-c2').title, goal: o('dt-c2').goal, position: 59, subtrack: 'electron', sections: o('dt-c2').sections },
    {
      id: 'dt-c4e', title: '桌面 · Electron 前端集成与构建优化',
      goal: '掌握如何将主流前端框架与 Electron 集成，并优化构建与打包流程。',
      position: 60, subtrack: 'electron',
      sections: [
    ...
    { id: 'dt-c3', title: o('dt-c3').title, goal: o('dt-c3').goal, position: 63, subtrack: 'tauri', sections: o('dt-c3').sections },
    {
      id: 'dt-c4t', title: '桌面 · Tauri 前端集成与构建优化',
      goal: '掌握如何将主流前端框架与 Tauri 集成，并优化构建与打包流程。',
      position: 64, subtrack: 'tauri',
      sections: [
...
// 幂等：若种子已含拆分后的章节，则跳过变换，直接取现有 8 章用于同步 DB
function transformSeed() {
  ...
  const merged = [...others, ...newChapters].sort((a, b) => (a.position ?? 0) - (b.position ?? 0))
  mod.chapters = merged
...
function applyDb(newChapters) {
  ...
  const before = db.prepare("SELECT COUNT(*) c FROM chapters WHERE module_id='frontend' AND subtrack IN ('desktop','electron','tauri')").get().c
  ...
  tx()
  const after = db.prepare("SELECT COUNT(*) c FROM chapters WHERE module_id='frontend' AND subtrack IN ('desktop','electron','tauri')").get().c
...
if (DRY) {
  console.log(alreadySplit ? '[dry-run] 种子已拆分，将同步以下 8 章到 DB:' : '[dry-run] 将生成的 8 章:')
  ...
  if (!alreadySplit) { fs.writeFileSync(SEED_PATH, JSON.stringify(content)); console.log('[seed] 已写入拆分后的 8 章') }
