<template>
  <div>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-7">
      ...
      <a-card :body-style="{ padding: '16px' }" class="flex items-center gap-3">
    ...
    <div v-if="!modulesTree.length" class="grid md:grid-cols-3 gap-5 stagger">
      <a-card v-for="i in 3" :key="i"><a-skeleton active :paragraph="{ rows: 6 }" /></a-card>
    </div>
          ...
          <div class="min-w-0 flex-1">
            ...
            <div class="mt-3.5">
              ...
              <div class="flex flex-wrap gap-1.5">
                <a-tag v-for="x in notCovered" :key="x" class="!border-line !bg-surface !text-muted !font-medium" :bordered="false">{{ x }}</a-tag>
              </div>
            </div>
          </div>
        </div>
