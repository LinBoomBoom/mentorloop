<template>
  <div>
    <div class="mb-1">
      ...
      <p class="text-muted text-sm mt-1">收录自用户提问中「题库未命中」的问题，已由大模型做语义化增强。审核通过即回流进正式面试题库；无收录价值的直接驳回。</p>
    </div>
    ...
    <a-card :body-style="{ padding: '0' }" class="overflow-x-auto">
      <table class="w-full text-sm">
        <thead><tr class="text-left text-muted border-b border-line">
          ...
          <th class="p-3">增强标题</th><th class="p-3">标签</th><th class="p-3">AI 答案预览</th><th class="p-3 text-right">操作</th>
        </tr></thead>
        <tbody>
          <tr v-for="item in items" :key="item.id" class="border-b border-line last:border-0 align-top">
          ...
          <tr v-if="!items.length"><td colspan="7" class="p-6 text-center text-muted">暂无数据</td></tr>
        </tbody>
      </table>
    </a-card>
    ...
    <a-card v-if="editor.open" :body-style="{ padding: '24px' }" class="mt-5">
      ...
      <p class="text-xs text-muted mb-3">以下为 AI 语义化增强后的内容，可在采纳前微调。确认后写入正式面试题库。</p>
