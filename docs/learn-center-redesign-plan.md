...
- **P4 题库展示归并**：`InterviewBank` 改读 `LEARNING_TAXONOMY`，验证三处一致。
  - **实际执行（2026-08-19）**：①`app/components/InterviewBank.vue` 重写为基于 `LEARNING_TAXONOMY` 渲染——大类 chip 组（flex-wrap，**无横向滚动条**）+ 子方向（`nav` 单选 / `filter` 多选）；方向点击经 `skillSubtrack→subtrack` 或 `techName→tech` 传给 API；`official` 方向显示占位引导卡；移除旧"按技能树/按技术筛选"双模式切换。②`learningTaxonomy.ts` 的 `Direction` 新增 `skillSubtrack?`（路线图 subtrack id），并为各方向填入映射（鸿蒙→fe-harmony、跨端→fe-app、原生→fe-native、桌面→fe-desktop、可视化→fe-viz、移动端→fe-mobile、小程序综合→fe-miniprogram、uni-app→fe-uniapp；后端 be-web-java→be-web、be-arch-micro→be-micro、be-data-mysql→be-db、be-data-mongodb→be-data；运维 k8s→op-k8s、sre→op-sre、cicd→op-trad、docker→op-devops；AI prompt/rag/agent→ai-app、eval→ai-mlops、deploy→ai-infra），并加 `groupSubtracks/groupTechs` 辅助函数。③API `server/api/interview/[track].get.ts` 扩展为支持逗号分隔 `subtrack`/`tech` 且两者并存时 OR 合并（大类级联合筛选），向后兼容单值。④**顺带修复 P1 回归**：`server/utils/interviewSlugs.ts` 的 `TECH_SLUG_MAP` 仍是归一化前的旧 tech 名，导致 `/interview/[track]/[tech]/[qid]` 详情路由对中文 tech 失效；已重建为 27 个归一化 tech + 4 个 official techName 的显式 slug 映射。
  - **验证**：`nuxt dev` 下 `/interview/{frontend,backend,devops,ai}` 均 SSR 200；新分类标记（方向大类/小程序/鸿蒙…）齐全、旧树模式标记（按技能树/按技术筛选）清零；API 组级联合 `subtrack=fe-miniprogram,fe-uniapp`→147 题、单方向 `tech=React`→146、`subtrack=fe-harmony`→89 均正确；详情 slug 路由 `react`/`engineering`(工程化)/`performance`(性能优化) 均 200。唯一 dev 日志噪声为 `.nuxt` 写权限 EPERM 竞态（环境，非代码缺陷）。
