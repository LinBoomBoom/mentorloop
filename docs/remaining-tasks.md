...
| 内容基座 | 知识树4模块全建(547节)+题库6565+考卷19 | C2/C3溯源验收 | 后端/AI格式升级(已完成) |
| VIP 价值线 | 1（呈现层） | 11 | ①支付 ②LLM |
| 前端体验 | 局部 | 4 | — |
| 管理后台 | 0 | 7 | — |

> 合计待办约 50 项（内容基座由 14 项收敛至 2 项：C2/C3 溯源验收）；四模块知识树 547 节已 100% 采用 `(可溯源)` 块格式（后端/AI 格式升级已于 08-16 收尾，commit 18f1700）。最大卡点仍为 3 个资质决策（支付 / LLM / OAuth）。安全/数据库/部署等看板项于 08-16 同步，如需可单独立项复核。
