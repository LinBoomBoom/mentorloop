...
- [x] C2 验收（commit 见下，报告 `docs/c2-c3-provenance-report.md`）：新增 `source` 字段，6565 题中 **4835 题（73.6%）带可追溯源**（104 精确 URL + 4731 tech 枢纽源）；1730 题（`rq-*` 路线图题）无单一官方源留 null 交专家锚定；抽检 15 题事实正确，发现 2 类粗粒度 tech 标签错配（内容质量任务，另案处理）。DB 已建 `source` 列（db.ts v20 迁移）+ reseed 同步
...
| 内容基座 | 知识树4模块全建(547节)+题库6565(73.6%可溯源)+考卷19 | ✅ 全完成（C2/C3 溯源验收 + 后端/AI格式升级） | — |
| VIP 价值线 | 1（呈现层） | 11 | ①支付 ②LLM |
| 前端体验 | 局部 | 4 | — |
| 管理后台 | 0 | 7 | — |

> 合计待办约 50 项（**内容基座 Phase 0 全部完成**：知识树 547 节 100% `(可溯源)` 块格式 + 题库 6565 题 73.6% 带结构化 `source` 源 + 考卷 19 套已预留 `source` 列待专家锚定；C2/C3 溯源验收与后端/AI 格式升级均于 08-16 收尾，报告见 `docs/c2-c3-provenance-report.md`）。最大卡点仍为 3 个资质决策（支付 / LLM / OAuth）。安全/数据库/部署等看板项于 08-16 同步，如需可单独立项复核。
