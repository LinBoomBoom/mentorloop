...
> 合计待办约 36 项（不含 5 项决策阻塞）。**内容基座 Phase 0 全部完成**（知识树 547 节 100% `(可溯源)` 块格式 + 题库 6565 题 73.6% 带结构化 `source` + 考卷 19 套预留 `source` 列；报告见 `docs/c2-c3-provenance-report.md`）。最大卡点仍为 5 项决策（支付 / LLM / OAuth / 续费策略 / 部署形态）。安全(B)/数据库/部署(C) 看板项已对齐现状；此前"3/7""~16"等旧数为 08-16 同步时的口径，本次已校正。
...
9. G1–G7 管理后台 CRUD + 用户/VIP/订单/看板
